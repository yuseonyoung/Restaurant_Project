...
//----------------------------------------------------------           堅   
   ...
   public List<Map<String, Object>> selectWeeklySalesList(List<Object> param){
      String sql =  " SELECT "
               + "    MIN(SUBSTR(A.O_ID,1,8)) AS MIN_ONEDAY"
               + "    , MAX(SUBSTR(A.O_ID,1,6)) AS MAX_ONEDAY"
               + "    , TO_CHAR(TO_DATE(SUBSTR(A.O_ID,1,8),'YYMMDD'),'IW')||'  ' AS WEEK" 
               + "    , SUM(A.OF_QTY * B.F_PRICE) AS GAIN" 
               + " FROM O_FOOD A" 
               + "   INNER JOIN FOOD B ON A.F_ID = B.F_ID" 
               + " WHERE SUBSTR(A.O_ID,1,6) BETWEEN ? AND ?" //泥ル쾲吏� ? : �떆�옉�씪   / �몢踰덉㎏? : 醫낅즺�씪
               + " GROUP BY TO_CHAR(TO_DATE(SUBSTR(A.O_ID,1,8),'YYMMDD'),'IW')" 
               + " ORDER BY TO_CHAR(TO_DATE(SUBSTR(A.O_ID,1,8),'YYMMDD'),'IW')";
      return (List<Map<String, Object>>) jdbc.selectList(sql,param);
   }
   ...
   public Map<String ,Object> selectEmpSal(){
      ...
      return (Map<String ,Object>) jdbc.selectChoice(sql);
   }
